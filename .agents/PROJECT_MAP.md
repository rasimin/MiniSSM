...
- Editor SQL: Single Shared WebView2 instance dengan Monaco Editor multi-model architecture (`createTabModel`, `switchTabModel`, `disposeTabModel`). Autocomplete juga meresolusikan kolom dari alias tabel, CTE/derived table `SELECT *`, `APPLY`, dan alias table-valued function; metadata database mencakup kolom hasil table-valued function serta parameter routine.
...
| `Windows\MainWindow.xaml`, `Windows\MainWindow.xaml.cs` | Layout utama dan orkestrasi aplikasi: compact toolbar terintegrasi title bar (dengan caption buttons `🗕 🗖 ✕` di kanan & tombol Settings di sebelah Tools), menu Tools untuk query actions dan SQL Agent Monitor, area kosong title bar yang dapat digunakan untuk memindahkan window, Object Explorer, tab query, context menu, shortcut, open/save script, dan script object. Berisi single shared instance WebView2 (`SharedSqlEditorWebView`). |
| `Controls\QueryTabControl.xaml`, `Controls\QueryTabControl.xaml.cs`, `Controls\QueryTabControl.Execution.cs` | Layout dan logic satu tab query: WebView2 editor, splitter, Results/Messages tab (dengan pesan berwarna & double-click error jump ke baris Monaco Editor), eksekusi query dengan **Safety Guardrail konfirmasi UPDATE/DELETE tanpa WHERE**, streaming pesan `InfoMessage` secara realtime melalui buffer/flush berkala agar ribuan output tidak membebani UI, pembatalan query asynchronous agar UI tidak hang saat `SqlCommand.Cancel()`, result grid, dan cache autocomplete metadata (termasuk Foreign Keys). |
| `Services\DatabaseHelper.cs` | Semua akses SQL Server: metadata database/object, eksekusi query, format error SQL dengan deduplikasi pesan identik, dan generate script. |
...
| `Windows\SqlAgentWindow.xaml`, `Windows\SqlAgentWindow.xaml.cs` | Window dark-mode SQL Agent Monitor untuk melihat status service Agent, daftar job, auto-refresh, start/stop job, enable/disable job, serta tab Steps, Schedules, dan History/Error dari database `msdb`. |
| `Services\SqlAgentService.cs` | Akses metadata SQL Server Agent melalui `msdb`: status service, job, step, schedule, history, serta operasi start/stop dan enable/disable job. |
...
| `Windows\SqlTraceWindow.xaml`, `Windows\SqlTraceWindow.xaml.cs` | Form warning dan monitoring real-time legacy SQL Trace/Profiler dengan header minimize/maximize/close, filter database, tombol Start/Stop, grid event query, serta panel pencarian real-time (Ctrl+F) untuk memfilter hasil trace (SQL text, database, login, event, SPID) saat status trace dihentikan (stopped). |
...
| `Editor\sql_editor_autocomplete.js` | Helper parsing statement aktif, alias/source query (termasuk scope CTE, derived table, wildcard `SELECT *`, table-valued function, dan proteksi reserved keywords / table hint seperti `WITH (NOLOCK)` agar tidak dianggap alias), inferensi kolom CTE dari metadata sumber, lookup kolom, pencocokan Smart Auto-JOIN (Foreign Keys & kesamaan nama kolom tanpa constraint FK), metadata lintas database, dan hover object/column info. |
...
18. Menu `Tools > SQL Agent Monitor` membuka `SqlAgentWindow` menggunakan koneksi tab aktif, sedangkan menu konteks instance Object Explorer membukanya menggunakan koneksi instance yang diklik; memonitor status Agent, dan membaca/menjalankan operasi job melalui database `msdb`; detail job menyediakan tab Steps, Schedules, dan History.
