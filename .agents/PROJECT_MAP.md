...
| `Editor\sql_editor_autocomplete.js` | Helper parsing statement aktif, alias/source query (termasuk scope CTE, derived table, wildcard `SELECT *`, dan table-valued function), inferensi kolom CTE dari metadata sumber, lookup kolom, pencocokan Smart Auto-JOIN (Foreign Keys & kesamaan nama kolom tanpa constraint FK), metadata lintas database, dan hover object/column info. |
